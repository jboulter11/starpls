use crate::{
    ...
    typeck::{
        ...
        FileLoadItemId, FileLoadStmt, FileParamId, Protocol, Substitution, Tuple, Ty, TyCtxt,
            ...
            Some((execution_scope, defs)) => {
                ...
                for def in defs.skip_while(|def| def.scope > expr_scope) {
                    let ty = match def.def {
                        ScopeDef::Variable(VariableDef { expr, source, .. }) => {
                            var_defs.push((*expr, source.clone()));
                            continue;
                        }
                ...
                let effective_ty = Ty::union(var_defs.into_iter().map(|(expr, source)| {
                    let cached_ty = self
                        .cx
                        .type_of_expr
                        .get(&FileExprId::new(file, expr))
